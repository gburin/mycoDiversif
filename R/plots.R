...
r.vasc.stem.r0 <- bd.ms(time = stem.age.vasc, n = sum(family.data.gen.valid$rich), crown = FALSE, epsilon = 0)
r.vasc.stem.r05 <- bd.ms(time = stem.age.vasc, n = sum(family.data.gen.valid$rich), crown = FALSE, epsilon = 0.5)
r.vasc.stem.r0.9 <- bd.ms(time = stem.age.vasc, n = sum(family.data.gen.valid$rich), crown = FALSE, epsilon = 0.9)
...
ggplot(data = limits.vasc.stem) +
    ...
    geom_point(data = family.data.gen.valid, aes(x = stem.age, y = rich, colour = type.60), size = 2.5) +
    geom_text_repel(data = family.data.gen.valid, aes(x = stem.age, y = rich, label = family, colour = type.60.valid)) +
    scale_y_log10() +
...
family.data.gen.valid$type.50.valid <- as.character(family.data.gen.valid$type.50.valid)
family.data.gen.valid$type.60.valid <- as.character(family.data.gen.valid$type.60.valid)
family.data.gen.valid$type.80.valid <- as.character(family.data.gen.valid$type.80.valid)
family.data.gen.valid$type.100.valid <- as.character(family.data.gen.valid$type.100.valid)

box.r0 <-
    ggplot(data = na.omit(family.data.gen.valid)) +
    geom_point(aes(x = factor(type.60.valid, levels = c("AM", "EM", "NM", "ER", "MIX")), y = r.e0, colour = factor(type.60.valid, levels = c("AM", "EM", "NM", "ER", "MIX")), size = shannon), alpha = 0.75, position = position_jitterdodge(jitter.width = 0.75, dodge.width = 1)) +
    geom_boxplot(aes(x = factor(type.60.valid, levels = c("AM", "EM", "NM", "ER", "MIX")), y = r.e0), fill = NA, colour = "darkgrey", outlier.alpha = 1) +
...
box.r09 <-
    ggplot(data = na.omit(family.data.gen.valid)) +
    geom_point(aes(x = factor(type.60.valid, levels = c("AM", "EM", "NM", "ER", "MIX")), y = r.e09, colour = factor(type.60.valid, levels = c("AM", "EM", "NM", "ER", "MIX")), size = shannon), alpha = 0.75, position = position_jitterdodge(jitter.width = 0.75, dodge.width = 1)) +
    geom_boxplot(aes(x = factor(type.60.valid, levels = c("AM", "EM", "NM", "ER", "MIX")), y = r.e09), fill = NA, colour = "darkgrey", outlier.alpha = 1) +
    theme(legend.position = "none") +
    scale_colour_brewer(palette = "Set1") +
