...
here::i_am("R/data_prep_bamm_smith_brown.R")

# Adapted from Igea & Tanentzap 2020 Ecol Lett

tree <- GBOTB.extended
write.tree(GBOTB.extended, file = here::here("output/GBOTB_extended.tree"))
...
is.binary(tree.bif)
write.tree(tree.bif, file = here::here("output/GBOTB_extended_bif.tree"))

#drop species that are not angiosperms and on taxonlookup
GBOTB.lookup <- lookup_table(tree.bif$tip.label, by_species = TRUE, include_counts = TRUE)
...
#drop species that are not spermatophytes and on taxonlookup
GBOTB.bif.sperm <- drop.tip(tree.bif, tip = setdiff(tree.bif$tip.label, rownames(GBOTB.lookup)[GBOTB.lookup$group != "Pteridophytes"]))
GBOTB.lookup.sperm <- GBOTB.lookup[GBOTB.lookup$group != "Pteridophytes",]

#n = 72986 angiosperms, GBOTB has been through TPL
write.tree(GBOTB.bif.angios, file = here::here("output/GBOTB_extended_bif_angios.tree"))

#n = 73832 spermatophytes, GBOTB has been through TPL
write.tree(GBOTB.bif.sperm, file = here::here("output/GBOTB_extended_bif_sperm.tree"))

#add species names to table
GBOTB.lookup.angios$species <- rownames(GBOTB.lookup.angios)
GBOTB.lookup.sperm$species <- rownames(GBOTB.lookup.sperm)
rownames(GBOTB.lookup.angios) <- NULL
rownames(GBOTB.lookup.sperm) <- NULL

#get families and number of species per family
PlantLookup <- plant_lookup(include_counts = TRUE)
PlantLookup <- PlantLookup[PlantLookup$group %in% c("Angiosperms", "Gymnosperms"),]
...
table.sampling <- merge(GBOTB.lookup.sperm, PlantLookup.2, by = "family", all.x = TRUE)

#count number of species in each family on the tree
family.count.table <- as.data.frame(table(table.sampling$family), stringsAsFactors = FALSE)
...
tree <- drop.tip(GBOTB.bif.sperm, setdiff(GBOTB.bif.sperm$tip.label, table.sampling$species))
# 73422 species
Family_BAMM_Sampling <- table.sampling[,c("species", "family", "family.sampling.fraction")]
write.table(Family_BAMM_Sampling, file = here::here("output/bamm_smith_brown/tables/GBOTB_BAMM_family_sampling.txt"))
...
selected.species <- vector(mode = "numeric", length = nrow(node.ages.5000))
selected.nodes <- vector(mode = "numeric", length = nrow(node.ages.5000))
selected.lengths <- vector(mode = "numeric", length = nrow(node.ages.5000))
for (i in 1:nrow(node.ages.5000)){
  ...
  if(length(intersect(descendant.species, selected.species)) == 0){
    selected.species[i] <- descendant.species
    selected.nodes[i] <- node.ages.5000[i, "parental.node"]
    selected.lengths[i] <- length(descendant.species)
  }
  cat(sum(selected.lengths),"\n")
  if(sum(selected.lengths, na.rm = TRUE) == length(tree$tip.label)){
    cat("done","\n")
    break
  }
}
...
species.selected.length <- species.selected.length[order(order.vector)]
...
selected.species <- vector(mode = "numeric", length = nrow(node.ages.6000))
selected.nodes <- vector(mode = "numeric", length = nrow(node.ages.6000))
selected.lengths <- vector(mode = "numeric", length = nrow(node.ages.6000))
...
for (i in 1:nrow(node.ages.6000)){
  ...
  if(length(intersect(descendant.species, selected.species)) == 0){
    selected.species[i] <- descendant.species
    selected.nodes[i] <- node.ages.6000[i, "parental.node"]
    selected.lengths[i] <- length(descendant.species)
  }
  cat(sum(selected.lengths, na.rm = TRUE), "\n")
...
for (i in 1:length(BAMM.clades.to.run)){
  setBAMMpriors(BAMM.clades.to.run[[i]], outfile = here::here(paste0("output/bamm_smith_brown/trees/GBOTB_clades/clade_", i, "_priors.txt")))
}

#write treefiles
for (i in 1:length(BAMM.clades.to.run)){
  write.tree(BAMM.clades.to.run[[i]], file = here::here(paste0("output/bamm_smith_brown/trees/GBOTB_clades/clade_", i, ".tree")))
}

#write sampling fractions,first backbones
for (i in 1:length(BAMM.clades.sampling.df)){
  write.table(BAMM.clades.sampling.backbone.fractions[i], file = here::here(paste0("output/bamm_smith_brown/trees/GBOTB_clades/clade_", i, "_sampling.txt")), quote = FALSE, row.names = FALSE,sep = "\t", col.names = FALSE)
}

#write sampling fractions,then table
for (i in 1:length(BAMM.clades.sampling.df)){
  write.table(BAMM.clades.sampling.df[[i]], file = here::here(paste0("output/bamm_smith_brown/trees/GBOTB_clades/clade_", i, "_sampling.txt")), quote = FALSE, row.names = FALSE, sep = "\t", col.names = FALSE, append = TRUE)
}
