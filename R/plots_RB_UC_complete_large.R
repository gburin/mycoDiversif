...
RB.tip.list <- RB.tree.UC.complete$tip.label
...
age.data$familia[is.na(match(age.data$familia, rb.UC.complete.ages$Family))]

## Adding ages from Ramírez-Barahona et al. 2020
age.data$rb.UC.complete.stem <- rb.UC.complete.ages$Stem_BEAST[match(age.data$familia, rb.UC.complete.ages$Family)]

## Importing results from random datasets
load("../output/RB_results_UC_complete_large.RData")
nrep <- 10000
fullresults <- ldply(1:nrep, function(x){results.UC.complete.large[[x]][[1]]})
...
family.data.gen$stem.age <- age.data$rb.UC.complete.stem[match(family.data.gen$family, age.data$familia)]
...
ggsave(filename = "../output/figs/scatterplots_lm_pgls_stem_r09_UC_complete_large.pdf", height = 5.5, width = 13)
...
ggsave(filename = "../output/figs/scatterplots_lm_pgls_stem_age_UC_complete_large.pdf", height = 5.5, width = 13)
...
ggsave(filename = "../output/figs/scatterplots_lm_pgls_stem_rich_UC_complete_large.pdf", height = 5.5, width = 13)


### Boxplots
## We aggregate mean rates per type for each random dataset

age.data$r.e0 <- geiger::bd.ms(time = age.data$rb.UC.complete.stem, n = age.data$nro_especies, crown = FALSE, epsilon = 0)
age.data$r.e09 <- geiger::bd.ms(time = age.data$rb.UC.complete.stem, n = age.data$nro_especies, crown = FALSE, epsilon = 0.9)
...
ggsave(filename = "../output/figs/boxplots_anova_myctype_r09_UC_complete.pdf", box.r09, width = 11, height = 5.5, units = "in")
...
ggsave(filename = "../output/figs/box_hist_anova_myctype_r09_UC_complete_large.pdf", width = 11, height = 9, units = "in")
...
ggsave(filename = "../output/figs/jitter_pvalue_anovas_r09_UC_complete_large.pdf", anovas.r09.pvalue, width = 11, height = 5.5, units = "in")
