...
RB.tip.list <- RB.tree.CC.conservative$tip.label
...
age.data$familia[is.na(match(age.data$familia, rb.CC.conservative.ages$Family))]

## Adding ages from Ramírez-Barahona et al. 2020
age.data$rb.CC.conservative.stem <- rb.CC.conservative.ages$Stem_BEAST[match(age.data$familia, rb.CC.conservative.ages$Family)]

## Importing results from random datasets
load("../output/RB_results_CC_conservative_large.RData")
nrep <- 10000
fullresults <- ldply(1:nrep, function(x){results.CC.conservative.large[[x]][[1]]})
...
family.data.gen$stem.age <- age.data$rb.CC.conservative.stem[match(family.data.gen$family, age.data$familia)]
...
ggsave(filename = "../output/figs/scatterplots_lm_pgls_stem_r09_CC_conservative_large.pdf", height = 5.5, width = 13)
...
ggsave(filename = "../output/figs/scatterplots_lm_pgls_stem_age_CC_conservative_large.pdf", height = 5.5, width = 13)
...
ggsave(filename = "../output/figs/scatterplots_lm_pgls_stem_rich_CC_conservative_large.pdf", height = 5.5, width = 13)


### Boxplots
## We aggregate mean rates per type for each random dataset

age.data$r.e0 <- geiger::bd.ms(time = age.data$rb.CC.conservative.stem, n = age.data$nro_especies, crown = FALSE, epsilon = 0)
age.data$r.e09 <- geiger::bd.ms(time = age.data$rb.CC.conservative.stem, n = age.data$nro_especies, crown = FALSE, epsilon = 0.9)
...
ggsave(filename = "../output/figs/boxplots_anova_myctype_r09_CC_conservative.pdf", box.r09, width = 11, height = 5.5, units = "in")
...
ggsave(filename = "../output/figs/box_hist_anova_myctype_r09_CC_conservative_large.pdf", width = 11, height = 9, units = "in")
...
ggsave(filename = "../output/figs/jitter_pvalue_anovas_r09_CC_conservative_large.pdf", anovas.r09.pvalue, width = 11, height = 5.5, units = "in")
