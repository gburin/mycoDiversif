...
nrep = length(list.files("./output/20perc_data/"))

random.analysis <- function(x, age, fulltree){
    print(paste0("Replica ", x))
    family.data.gen <- read.csv(paste0("./output/20perc_data/", list.files("./output/20perc_data/")[x]), stringsAsFactors = FALSE)
...
random.results <- ldply(1:2, random.analysis, age = age.data, fulltree = fulltree, .parallel = TRUE)
