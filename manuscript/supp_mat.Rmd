...
# Analyses with the Species-level database
...
### Boxplots using the clean species-level database

```{r fig.height = 11, fig.cap = "Relationship between mycorrhizal type and diversification rates using the thresholds (a) 50%, (b) 60%, (c) 80% and (d) 100% for MIX state assignment. Each panel shows diversification rate estimated with epsilon = 0 (upper) and diversification rate estimated with epsilon = 0.9 (lower). AM: Arbuscular mycorrhiza, EM: Ectomycorrhiza, NM: non-mycorrhizal and MIX (families with no dominance of any specific mycorrhizal association). The size of the points indicates the Mycorrhizal Type Diversity Index value for each lineage, indicating a predominance of larger indices with higher diversification rates."}
...
### Summary statistics using clean species-level database
...
### Posthoc tests using clean species-level database
...
```{r fig.height = 5, fig.cap = "Scatterplots showing the relationship between mycorrhizal diversity index and diversification rates. Diversification rates were estimated with epsilon = 0 (a) and with epsilon = 0.9 (b). The red and blue lines indicate the results of a linear model and a phylogenetic generalized least squares (PGLS) fit, respectively."}
...
## Full species-level database - including species with any remark
...
### Boxplots using full species-level database
...
### Summary statistics using full species-level database
...
### Posthoc tests using full species-level database
...
# Analyses with the genus-level database
...
### Boxplots using clean genus-level database

```{r fig.height = 11, fig.cap = "Relationship between mycorrhizal type and diversification rates. a) diversification rate estimated with epsilon = 0 and b) diversification rate estimated with epsilon = 0.9. AM: Arbuscular mycorrhiza, EM: Ectomycorrhiza, NM: non-mycorrhizal and MIX (families with no dominance of any specific mycorrhizal association). The size of the points indicates the Mycorrhizal Type Diversity Index value for each lineage, indicating a predominance of larger indices with higher diversification rates."}
...
### Summary statistics using clean genus-level database
...
## Genus-level full database - including all families
...
### Boxplots using genus-level full database
...
### Posthoc tests using genus-level full database
...
### Scatterplots using genus-level full database
...
kable_styling(kable(x = data.frame(Variable = c("r.e0", "r.e09", "Stem.Age", "Richness"),
                    ...
                    format = "latex", caption = "Phylogenetic signal of the four response variables. Significant values are highlighted in bold.", escape = FALSE), latex_options = "HOLD_position"
              )
