...
library("plyr")
library("here")
here::i_am("manuscript/supp_mat_2021.Rmd")
rm(list = ls())
...
```{r fig.cap = "Scatterplots showing the relationship between mycorrhizal diversity index and (a) stem age and (c) species richness. Panels (b) and (d) show the frequency distribution of R\\textsuperscript{2} and p-values PGLS. Dashed lines show the median for each distribution."}

## Age vs Shannon
scatter.age.sh <-
    ggplot(fullresults) +
    geom_abline(mapping = aes(intercept = pgls.int.age.sh, slope = pgls.slope.age.sh, colour = "PGLS"), alpha = 0.5) +
...
scatter.rich.sh <-
    ggplot(fullresults) +
    geom_abline(mapping = aes(intercept = pgls.int.rich.sh, slope = pgls.slope.rich.sh), colour = brewer.pal(3, "Set1")[1], alpha = 0.5) +
...
((scatter.age.sh | ((r2.pgls.age.sh + pvalue.pgls.age.sh))) + plot_annotation(tag_levels = 'A')) / ((scatter.rich.sh | ((r2.pgls.rich.sh + pvalue.pgls.rich.sh))))
...
# Appendix S3 - Randomly adding 20% of misassignment of mycorrhizal type into the data obtained from genus-level list, using rates from Igea & Tanentzap (20XX)
...
# Appendix S5 - Analyses with the Species-level dataset
...
raw.r0 <-
    ...
    geom_abline(data = data.frame(int = coef(mod.r0)[1], sl = coef(mod.r0)[2], col = "b"), mapping = aes(intercept = int, slope = sl, colour = col), size = 1.5) +
    labs(x = "Mycorrhizae Type Shannon Index", y = "Diversification Rate") +
    #xlim(0, 1) +
    theme_cowplot() +
...
raw.r09 <-
    ...
    geom_abline(data = data.frame(int = coef(mod.r09)[1], sl = coef(mod.r09)[2], col = "b"), mapping = aes(intercept = int, slope = sl, colour = col), size = 1.5) +
    labs(x = "Mycorrhizae Type Shannon Index", y = "Diversification Rate") +
    #xlim(0, 1) +
    theme_cowplot() +
    theme(legend.position = "none")
...
reg.sum <- data.frame(model = "PGLS",
                      pvalue.r0 = c(4.063e-07),
                      R2.r0 = c(0.08904),
                      pvalue.r09 = c(3.483e-07),
                      R2.r09 = c(0.09007))

reg.sum[, 2:5] <- apply(reg.sum[, 2:5], 2, sig.formatter)

kable(x = reg.sum, format = "latex", caption = "Summary statistics for the phylogenetic regression using the species-level dataset after removing species with remarks. Significant values are highlighted in bold.", escape = FALSE, digits = 3, col.names = c("Model", "p-value", "R\\textsuperscript{2}", "p-value", "R\\textsuperscript{2}"), align = "c") %>%
  kable_styling(#latex_options = "HOLD_position",
                full_width = F) %>%
  add_header_above(c(" ", "e = 0" = 2, "e = 0.9" = 2))
...
reg.sum <- data.frame(model = c("PGLS"),
                      pvalue.r0 = c(5.265e-05),
                      R2.r0 = c(0.06444),
                      pvalue.r09 = c(4.013e-05),
                      R2.r09 = c(0.06653))

reg.sum[, 2:5] <- apply(reg.sum[, 2:5], 2, sig.formatter)

kable(x = reg.sum, format = "latex", caption = "Summary statistics for the phylogenetic regressions using the species-level dataset with species with remarks. Significant values are highlighted in bold.", escape = FALSE, digits = 3, col.names = c("Model", "p-value", "R\\textsuperscript{2}", "p-value", "R\\textsuperscript{2}"), align = "c") %>%
  kable_styling(#latex_options = "HOLD_position",
                full_width = F) %>%
  add_header_above(c(" ", "e = 0" = 2, "e = 0.9" = 2))
...
# Appendix S6 - Analyses with the genus-level dataset
...
# Appendix S7 - Replicating main analyses with tree from Harris and Davies (2016)
...
# Appendix S8 - Phylogenetic signal of diversification rates, age, and richness
...
kable_styling(kable(summary(fullresults[, grep("lambda", names(fullresults))]), col.names = c("r (epsilon = 0)", "r (epsilon = 0.9)", "Stem Age", "Richness"), format = "latex", caption = "Summary statistics of phylogenetic signal signal of the four response variables used in the main analyses. For Stem Age and Richness there is only one value, since they do not change across datasets.", escape = FALSE), latex_options = "HOLD_position")

```


# Appendix S9 - Percentage of MIX species
...
# Appendix S10 - Replicating main analyses with all trees from Ramirez-Barahona \textit{et al.} (2020)

```{r rambarah_results, echo = FALSE, fig.cap = "Main analyses replicated using Ramirez-Barahona \\textit{et al.} (2020) phylogeny. The rates were estimated by the authors using the same source for species richness as the main analysis, and the stem ages were obtained from their phylogeny. Panels A: scatterplot with linear and phylogenetic regressions plotted; panel B: histograms of R2 and p-value of linear and phylogenetic regressions."}
...
```{r echo = FALSE, fig.cap = "Analysis of the relationship between age and MSDI replicated using Ramirez-Barahona \\textit{et al.} (2020) phylogeny. The rates were estimated by the authors using the same source for species richness as the main analysis, and the stem ages were obtained from their phylogeny. Panel A: scatterplots with linear and phylogenetic regressions plotted; panel B: histograms of R2 and p-value of linear and phylogenetic regressions."}
...
```{r echo = FALSE, fig.cap = "Analysis of the relationship between richness and MSDI replicated using Ramirez-Barahona \\textit{et al.} (2020) phylogeny. The rates were estimated by the authors using the same source for species richness as the main analysis, and the stem ages were obtained from their phylogeny. Panel A: scatterplots with linear and phylogenetic regressions plotted; panel B: histograms of R2 and p-value of linear and phylogenetic regressions."}
...
```{r echo = FALSE, fig.cap = "Distribution of p-values for the phylogenetic (red bars) ANOVA using different thresholds for classification. A: 50%, B: 60%, C: 80%, D: 100%."}
...
# Appendix S11 - Assigning random families a net diversification of 0 to test for limitations raised by Rabosky \& Benson 2021

## Substituting the diversification rates of 5% of the families by 0

```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0 of the PGLS analysis after substituting the diversification rates of 5% of the families by 0. Dashed line represents mean value."}
...
hist.pgls.r2.r0 <-
    ...
    labs(x = parse(text = "R^2"), y = element_blank()) +
    theme_cowplot()
...
hist.pgls.pvalue.r0 <-
    ...
    labs(x = "log10(p-value)", y = element_blank()) +
    theme_cowplot()
...
```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0.9 of the PGLS analysis after substituting the diversification rates of 5% of the families by 0. Dashed line represents mean value."}
...
## Substituting the diversification rates of 20% of the families by 0

```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0 of the PGLS analysis after substituting the diversification rates of 20% of the families by 0. Dashed line represents mean value."}
...
```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0.9 of the PGLS analysis after substituting the diversification rates of 20% of the families by 0. Dashed line represents mean value."}
...
## Substituting the diversification rates of 50% of the families by 0

```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0 of the PGLS analysis after substituting the diversification rates of 50% of the families by 0. Dashed line represents mean value."}
...
```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0.9 of the PGLS analysis after substituting the diversification rates of 50% of the families by 0. Dashed line represents mean value."}
...
# Appendix S12 - Assigning random families different values of epsilon to test for limitations raised by Rabosky \& Benson 2021

## Substituting the epsilon of 5% of the families by random value between [0, 1)

```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0 of the PGLS analysis after substituting the epsilon values of 5% of the families by a random value between \\[0, 1\\). Dashed line represents mean value."}

rm(list = ls())
load(here::here("output/side_analysis_rabosky_benson_epsilon_5perc.RData"))
...
hist.pgls.r2.r0 <-
    ggplot(data = side.results) +
    geom_histogram(aes(x = pgls.r2.r0), fill = "red", colour = NA, alpha = 0.3, bins = 100) +
    geom_vline(aes(xintercept = mean(pgls.r2.r0, na.rm = TRUE)), linetype = "dashed", size = 2, colour = "red") +
    labs(x = parse(text = "R^2"), y = element_blank()) +
    theme_cowplot()
...
hist.pgls.pvalue.r0 <-
    ggplot(data = side.results) +
    geom_histogram(aes(x = log10(pgls.pvalue.r0)), fill = "red", colour = NA, alpha = 0.3, bins = 100) +
    geom_vline(aes(xintercept = mean(log10(pgls.pvalue.r0 + 1e-16), na.rm = TRUE)), linetype = "dashed", size = 2, colour = "red") +
    #scale_x_continuous(breaks = seq(floor(min(log10(side.results$pgls.pvalue.r0 + 1e-16))), ceiling(max(log10(side.results$pgls.pvalue.r0 + 1e-16))), by = 2)) +
    labs(x = "log10(p-value)", y = element_blank()) +
    theme_cowplot()
...
hist.pgls.slope.r0 <-
    ggplot(data = side.results) +
    geom_histogram(aes(x = pgls.slope.r0), fill = "red", colour = NA, alpha = 0.3, bins = 100) +
    geom_vline(aes(xintercept = mean(pgls.slope.r0, na.rm = TRUE)), linetype = "dashed", size = 2, colour = "red") +
    labs(x = "Slope", y = element_blank()) +
    theme_cowplot()
...
```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0.9 of the PGLS analysis after substituting the epsilon values of 5% of the families by a random value between \\[0, 1\\). Dashed line represents mean value."}
...
## Substituting the epsilon of 20% of the families by random value between [0, 1)

```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0 of the PGLS analysis after substituting the epsilon values of 20% of the families by a random value between \\[0, 1\\). Dashed line represents mean value."}
...
```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0.9 of the PGLS analysis after substituting the epsilon values of 20% of the families by a random value between \\[0, 1\\). Dashed line represents mean value."}
...
## Substituting the epsilon of 50% of the families by random value between [0, 1)

```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0 of the PGLS analysis after substituting the epsilon values of 50% of the families by a random value between \\[0, 1\\). Dashed line represents mean value."}
...
```{r fig.cap = "Distribution of R\\textsuperscript{2}, p-value and slope value distributions for $\\epsilon$ = 0.9 of the PGLS analysis after substituting the epsilon values of 50% of the families by a random value between \\[0, 1\\). Dashed line represents mean value."}
